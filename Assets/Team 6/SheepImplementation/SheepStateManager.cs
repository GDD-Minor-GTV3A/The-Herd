...
namespace Core.AI.Sheep
{
    ...
    public class SheepStateManager : StateManager<IState>
    {
        ...
        private bool _stateLockedByPanic = false;

        public bool IsStateLocked => _stateLockedByPanic;
        ...
        public void DisableHerdParticipation()
        {
            Core.Events.EventManager.RemoveListener<Core.Events.PlayerSquareTickEvent>(OnPlayerSquareTick);
            Core.Events.EventManager.RemoveListener<Core.Events.PlayerSquareChangedEvent>(OnPlayerSquareChanged);
        }

        public void EnableHerdParticipation()
        {
            Core.Events.EventManager.AddListener<Core.Events.PlayerSquareTickEvent>(OnPlayerSquareTick);
            Core.Events.EventManager.AddListener<Core.Events.PlayerSquareChangedEvent>(OnPlayerSquareChanged);
        }
        
        public void LockStateFromExternal()
        {
            _stateLockedByPanic = true;
        }

        public void UnlockStateFromExternal()
        {
            _stateLockedByPanic = false;
        }
        ...
        public void ForceHerdResync()
        {
        if (_stateLockedByPanic) return;

        // Re-run herd logic once to immediately update position/target
        OnPlayerSquareChanged(default);
        OnPlayerSquareTick(default);
        }
    }
}
